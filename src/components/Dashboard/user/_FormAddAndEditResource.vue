...
export default defineComponent({
    props: ["isModalOpen", "onCloseModal", "objectToEdit", "isEditable"],
    ...
    mounted() {
        ...
        if (Object.keys(this.objectToEdit.data).length === 0) {
            ...
            this.adminResourceStoreStore.actionMainAdminResourceStore("ACTION_GET_ADMIN_RESOURCES", null);
            setTimeout(() => {
                this.selectValue = this.objectToEdit.data.id_resource_admin._id;
            }, 500);
        }
    },
    updated() {
        for (let i in this.objectToEdit.data.inputs_data) {
            if (i === "exp" || i === "iat") {
                delete this.objectToEdit.data.inputs_data[i]
            }
        }
        this.objectToCatch = this.objectToEdit.data.inputs_data
                ...
                setTimeout(() => {
                    ...
                    this.$refs.inputs_custom?.forEach((element: any, i: any) => {
                        if (this.$refs.inputs_custom[i].$el.name === "mesdeexpiracion") {
                            ...
                            this.$refs.inputs_custom[i].$el.maxLength = "4"
                        } else if (this.$refs.inputs_custom[i].$el.name === "descripcion") {
                            null
                        } else {
                        ...
                        if (Object.keys(this.objectToEdit.data).length === 0) {
                            ...
                            for (let i in this.objectToEdit.data.inputs_data) {
                                if (element?.$el?.name === i) {
                                    element.$el.value = this.objectToEdit.data.inputs_data[i];
                                }
                            }
                        }
                    });
    ...
    methods: {
          ...
          // console.log(e.target)
            if (e.target.type === "textarea" || e.target.type === "text" || e.target.type === "password" ||
                ...
                if (e.target.name === "mesdeexpiracion" || e.target.name === "aniodeexpiracion" ||
                    ...
                    } else if (/^([0-9])*$/.test(e.target.value) && e.target.name === "mesdeexpiracion") {
                      const num = Number(e.target.value);
                      if (num > 12) e.target.value = e.target.value.substring(0, e.target.value.length - 1);
                    }
                }
        ...
        handleSubmit() {
          const anio = document.querySelector('input[name="aniodeexpiracion"]') as HTMLInputElement
          if (anio){
            const mes = document.querySelector('input[name="mesdeexpiracion"]') as HTMLInputElement
            const numAnio = Number(anio.value);
            const numMes = Number(mes.value);
            if (numAnio < new Date().getFullYear()) {
              toast.error(numAnio === 0 ? "Año de expiración inválido" : "El año de expiración no puede ser menor al año actual", { position: "top-right", duration: 3500 });
              return
            } else if (numMes === 0) {
              toast.error("El mes de expiración no es válido", { position: "top-right", duration: 3500 });
              return
            } else {
              const anio = document.querySelector('input[name="aniodeexpiracion"]') as HTMLInputElement
              const date = anio.value+"/"+mes.value+"/"+new Date().getDay()
              if (new Date(date) < new Date()) {
                toast.error("No puede registrar una tarjeta caducada", { position: "top-right", duration: 3500 });
                return
              }
            }
          }
        ...
        //>>>>>>>>>>>>>>>>>>>
          ...
          if (Object.keys(this.objectToEdit.data).length > 0) {
            prop2 = {
              ...
              data: this.objectToCatch,
              name_resource: this.adminResourceStoreStore.DAPI_RESPONSE_ID.response?.data.name_resource
            }
          } else {
            prop2 = {
              id_resource_selected: Object.keys(this.objectToEdit.data).length > 0 ? this.objectToEdit.data._id : this.selectValue,
              name_resource: this.adminResourceStoreStore.DAPI_RESPONSE_ID.response?.data.name_resource,
              data: this.objectToCatch,
            }
          }
          this.userResourceStoreStore.actionMainUserResourceStore(ACTION, prop2, this.onCloseModal)
        }
    }
});
</script>
                ...
                <div
                    ...
                    <div class="card_modal px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            <div style="letter-spacing: 1px; margin-bottom:5%; font-weight: bold; font-size: 1.7em;">
                                {{ !objectToEdit.isEditable ? 'Ver recurso' : Object.keys(objectToEdit.data).length > 0 ? "Editar recurso" : "Agregar recurso" }}
                                <br />
                                <small>{{ Object.keys(objectToEdit.data).length > 0 ?
                                    objectToEdit.data.id_resource_admin.name_resource : "" }}</small>
                            </div>
                        </div>
                        <form class="pt-3 pb-3 mb-4">
                            <div v-if="Object.keys(objectToEdit.data).length === 0"
                                                    ...
                                                    <div v-if="i.type_input === 'password'">
                                                        <InputGroup>
                                                            <InputText class="input_data" ref="inputs_custom"
                                                                ...
                                                                :autocomplete="i.name_input + '_' + k" :disabled="!objectToEdit.isEditable" />
                                                            ...
                                                            <InputGroupAddon
                                                                v-if="i.name_input !== 'nrodetarjeta' && i.name_input !== 'codigodeseguridad' && objectToEdit.isEditable"
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                        <div v-if="selectValue" style="margin-left: 3%;">
                            <button v-if="objectToEdit.isEditable" type="button" class="BTN_SUCCESS_1 py-2 px-4 rounded"
                                @click="handleSubmit">
                                {{ Object.keys(objectToEdit.data).length > 0 ? 'Editar' : 'Agregar' }}
                            </button>
                        </div>
